...
const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  // Email is no longer editable/visible; keep only brand color here
  ...
  const handleSaveSettings = async () => {
    setIsSaving(true);
    try {
      // Keep existing adminEmail as-is; only update primaryColor
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            ...
            <div className="p-4 bg-slate-50 rounded-lg border border-slate-100">
              ...
              <div className="text-2xl font-bold text-green-600">
                {stats.filter(j => j.crm_status === 'synced').length}
              </div>
            </div>
          </div>
          ...
          {companyId ? (
            <div className="space-y-4">
              <p className="text-sm text-slate-600">
                Share this link with customers to start a branded inventory session.
              </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 h-full">
            ...
            <div className="space-y-6">
              {/* Destination email removed from UI on purpose */}
            ...
            <div className="flex justify-between items-start mb-4">
              <div>
                <h3 className="text-lg font-bold text-slate-800 mb-1 flex items-center gap-2">
                  <CloudLightning size={20} className="text-slate-400" /> CRM
                  Integration
                </h3>
                <p className="text-sm text-slate-500">
                  Connect to Supermove or Salesforce.
                </p>
              </div>
              <div
